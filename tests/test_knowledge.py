...
def test_query_by_tag(tmp_db_path):
    ...
    entry1 = KnowledgeEntry(
        id="TAG-001", topic="A", model_family="m1",
        finding="f1", details="d1", confidence=0.5,
        tags=["alpha", "beta"],
    )
    entry2 = KnowledgeEntry(
        id="TAG-002", topic="B", model_family="m2",
        finding="f2", details="d2", confidence=0.5,
        tags=["alpha", "gamma"],
    )
    kb.add_entry(entry1)
    kb.add_entry(entry2)
...
def test_query_by_confidence(tmp_db_path):
    ...
    for i in range(5):
        entry = KnowledgeEntry(
            id=f"CONF-{i:03d}", topic="Test", model_family="test_confidence",
            finding=f"f{i}", details="d", confidence=i * 0.25,
        )
        kb.add_entry(entry)
...
def test_export_json(tmp_db_path):
    ...
    entry = KnowledgeEntry(
        id="EXP-001", topic="Test", model_family="m",
        finding="test", details="test", confidence=0.5,
    )
    kb.add_entry(entry)
