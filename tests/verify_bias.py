...
class TestBias(unittest.TestCase):
    ...
    def test_diversity_mechanism(self):
        ...
        with (
            ...
            patch(
                "bioplausible.scientist.strategy.ScientistStrategy.TASK_WEIGHTS",
                mock_weights,
            ),
        ...
        cand_a_heavy = next(
            (c for c in candidates_heavy if c.task_name == "task_a"), None
        )
        cand_b_heavy = next(
            (c for c in candidates_heavy if c.task_name == "task_b"), None
        )
