...
def test_ep_mode_basic():
    ...
    print(f"  ✓ EP mode: loss={stats['loss']:.4f}, acc={stats['accuracy']:.4f}, beta={stats['beta']:.4f}")
    return True
...
def test_beta_annealing():
    ...
    assert abs(beta2 - expected_beta2) < 1e-6, f"Beta decay incorrect: {beta2} vs {expected_beta2}"
...
def test_mode_comparison_learning():
    ...
    assert pc_losses[-1] < pc_losses[0], f"PC should learn: {pc_losses[0]:.4f} → {pc_losses[-1]:.4f}"
...
def test_all_task_types():
    ...
    for mode in ['pc', 'ep']:
        for task_type, output_dim, y in task_configs:
            ...
            assert 'loss' in stats
            assert not torch.isnan(torch.tensor(stats['loss'])), f"{mode}/{task_type}: Loss is NaN"
