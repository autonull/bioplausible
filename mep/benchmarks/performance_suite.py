...
def benchmark_config(
    ...
    # Create model
    model = MLP(input_dim=784, hidden_dim=256, num_layers=depth, output_dim=10).to(
        device
    )
...
def run_performance_suite(epochs=3, quick=False):
    ...
    for cfg in configs:
        ...
        print(
            f"  Mode: {cfg['mode']}, Depth: {cfg['depth']}, Settle: {cfg['settle_steps']}, LR: {cfg['lr']}"
        )
        print(f"{'='*60}")
    ...
    valid_results = [
        r for r in results if not r.has_nan and not r.has_inf and r.test_accuracy > 0
    ]

    best_accuracy = (
        max(valid_results, key=lambda r: r.test_accuracy) if valid_results else None
    )
    fastest = (
        min(valid_results, key=lambda r: r.time_per_epoch_sec)
        if valid_results
        else None
    )
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, default="performance_results.json", help="Output file"
    )
