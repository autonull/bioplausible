...
class TestEPPerformanceBaseline:
    ...
    def mnist_train_loader(self):
        ...
        return DataLoader(
            Subset(train_dataset, range(1000)), batch_size=64, shuffle=True
        )
    ...
    def test_mnist_quick(self):
        ...
        assert (
            accuracy >= 0.80
        ), f"MNIST 1-epoch accuracy {accuracy:.2%} below threshold 80%"
    ...
    def test_mnist_extended(self):
        ...
        train_loader = DataLoader(
            Subset(train_dataset, range(2000)), batch_size=64, shuffle=True
        )
        test_dataset = datasets.MNIST("./data", train=False, transform=transform)
        test_loader = DataLoader(test_dataset, batch_size=256)
        ...
        assert (
            accuracy >= 0.88
        ), f"MNIST 3-epoch accuracy {accuracy:.2%} below threshold 88%"
...
class TestEPConfiguration:
    ...
    def test_default_beta(self):
        ...
        assert (
            default_beta >= 0.4
        ), f"Default beta {default_beta} too low, should be >= 0.4"
    ...
    def test_default_settle_steps(self):
        ...
        assert (
            default_steps >= 25
        ), f"Default settle_steps {default_steps} too low, should be >= 25"
    ...
    def test_default_settle_lr(self):
        ...
        assert (
            default_lr >= 0.1
        ), f"Default settle_lr {default_lr} too low, should be >= 0.1"
    ...
    def test_default_loss_type(self):
        ...
        assert (
            default_loss == "mse"
        ), f"Default loss_type should be 'mse', got '{default_loss}'"
