...
def run_gradient_comparison(
    model: nn.Module, x: torch.Tensor, y: torch.Tensor, loss_type: str = "mse"
    ...
    # Get EP gradients
    optimizer = smep(
        model.parameters(), model=model, mode="ep", loss_type=loss_type, settle_steps=10
    )
    ...
    for name, param in model.named_parameters():
        if param.requires_grad and param.numel() <= 100:  # Only small params for speed
            num_grad = calculate_numerical_gradient(
                model, x, y, param, loss_type=loss_type
            )
...
def test_numerical_gradients_cross_entropy(device):
    ...
    model = nn.Sequential(nn.Linear(4, 8), nn.ReLU(), nn.Linear(8, 3)).to(device)

    x = torch.randn(4, 4, device=device)
    y = torch.randint(0, 3, (4,), device=device)

    ep_grad, num_grad, cos_sim = run_gradient_comparison(
        model, x, y, loss_type="cross_entropy"
    )
