...
class TestPermutedMNIST:
    ...
    def test_different_seeds_different_permutations(self):
        ...
        different = any(
            not torch.equal(p1, p2) for p1, p2 in zip(b1.permutations, b2.permutations)
        )
        assert different
...
class TestMLP:
    ...
    def test_mlp_no_dropout(self, device):
        ...
        model = MLP(input_dim=50, hidden_dims=(32,), num_classes=5, dropout=0.0).to(
            device
        )
...
class TestOptimizer:
    ...
    def test_create_mep_optimizer_ep(self, simple_model):
        ...
        optimizer = create_mep_optimizer(
            simple_model, mode="ep", use_error_feedback=True
        )
    ...
    def test_create_mep_optimizer_backprop(self, simple_model):
        ...
        optimizer = create_mep_optimizer(
            simple_model, mode="backprop", use_error_feedback=False
        )
...
class TestTrainingLoop:
    ...
    def test_train_epoch(self, simple_model, sample_data, device):
        ...
        changed = any(
            ...
            for p, ip in zip(simple_model.parameters(), initial_params)
        )
        assert changed
