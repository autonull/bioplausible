...
def test_energy_function_validation(device):
    ...
    with pytest.raises(ValueError, match="Shape mismatch"):
        energy_fn(
            ...
            states,
            structure,
            target_vec=torch.randn(3, 5, device=device),
            beta=0.1,
        )
...
def test_settler_invalid_inputs(device):
    ...
    with pytest.raises(ValueError, match="Input tensor cannot be empty"):
        settler.settle(
            model, torch.tensor([], device=device), None, 0.0, energy_fn, structure
        )

    # 2. Invalid beta
    with pytest.raises(ValueError, match="Beta"):
        settler.settle(
            model, torch.randn(4, 10, device=device), None, 1.5, energy_fn, structure
        )
