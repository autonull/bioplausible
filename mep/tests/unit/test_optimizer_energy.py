...
def test_settle_energy_reduction(device):
    ...
    E_initial = energy_fn(
        model, x, initial_states, structure, target_vec=None, beta=0.0
    )

    # Settle
    settled_states = settler.settle(
        model, x, target=None, beta=0.0, energy_fn=energy_fn, structure=structure
    )

    # Compute settled energy
    E_settled = energy_fn(
        model, x, settled_states, structure, target_vec=None, beta=0.0
    )

    # Energy should decrease
    assert (
        E_settled.item() <= E_initial.item() + 1e-5
...
def test_classification_energy(device):
    ...
    model = nn.Sequential(nn.Linear(10, 5), nn.ReLU(), nn.Linear(5, 3)).to(device)
...
def test_energy_complex_layers(device):
    ...
    attention = nn.MultiheadAttention(embed_dim, num_heads=2, batch_first=True).to(
        device
    )
