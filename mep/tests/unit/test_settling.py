...
def test_settle_adaptive_stopping(device):
    ...
    model = nn.Sequential(nn.Linear(10, 10), nn.Tanh(), nn.Linear(10, 2)).to(device)
    ...
    settler = Settler(steps=max_steps, lr=0.05, tol=tol, patience=patience)
    ...
    assert (
        call_count < max_steps
    ), f"Settler should have stopped early (steps: {call_count} vs max: {max_steps})"
    assert (
        call_count > patience
    ), f"Settler should run at least patience steps (steps: {call_count})"
...
def test_settle_with_graph_adaptive(device):
    ...
    assert (
        call_count < max_steps
    ), f"Settler should have stopped early (steps: {call_count})"
