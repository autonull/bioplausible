...
from mep.optimizers import O1MemoryEPv2, analytic_state_gradients, settle_manual_o1
...
def verify_analytic_gradients(
    ...
    grads_autograd = torch.autograd.grad(
        E_autograd, states_for_autograd, retain_graph=False
    )

    # Compute gradients analytically
    grads_analytic = analytic_state_gradients(
        model, x, states, structure, target_vec, beta, loss_type="cross_entropy"
    )
...
def print_memory_results(results: List[MemoryComparison]):
    ...
    print("=" * 100)
    print(
        f"{'Depth':<8} {'Current EP (MB)':<18} {'O(1) v2 (MB)':<15} {'Savings (MB)':<15} {'Savings (%)':<12} {'Time Ratio':<10}"
    )
    print("-" * 100)
    ...
    for r in results:
        ...
        print(
            f"{r.depth:<8} {r.current_ep_mb:<18.2f} {r.o1_ep_v2_mb:<15.2f} {r.savings_mb:<15.2f} {r.savings_percent:>10.1f}% {time_ratio:>8.2f}x"
        )
...
def main():
    ...
    print(
        f"  Difference: {grad_result.difference:.6e} (tolerance: {grad_result.tolerance:.6e})"
    )
    ...
    settle_result = verify_settle_analytic(
    ...
    training_result, time_current, time_o1 = verify_o1_v2_training(
        model, x, y, structure
    )
    ...
    print(
        f"  Time: Current={time_current:.3f}s, O(1) v2={time_o1:.3f}s ({time_o1/time_current:.2f}x)"
    )
    ...
    all_correctness_passed = all(
        [grad_result.passed, settle_result.passed, training_result.passed]
    )
    ...
    print("\nSuccess Criteria:")
    print(
        f"  1. Analytic gradients <1e-5 difference: {'✓' if grad_result.passed else '✗'}"
    )
    print(
        f"  2. Analytic settling <1e-5 difference:  {'✓' if settle_result.passed else '✗'}"
    )
    print(
        f"  3. Memory savings 50%+ at depth 500:    {'✓' if memory_target_met else '✗'}"
    )
