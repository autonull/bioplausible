...
def run_benchmark(
    ...
    for method_name, use_ep, use_ewc in methods:
        ...
        for task_id in range(num_tasks):
            ...
            print(
                f"  Task {task_id}: Train acc = {accs[-1]*100:.1f}%, "
...
def print_results_table(summary: BenchmarkSummary):
    ...
    for r in summary.results:
        print(
            f"{r.method:<15} {r.average_accuracy*100:<15.1f} "
...
def save_results(
    summary: BenchmarkSummary, filename: str = "continual_learning_results.json"
):
...
def main():
    ...
    if ep_ewc.forgetting_measure < 0.15:
        ...
        print(
            f"  ⚠️  EP + EWC has {ep_ewc.forgetting_measure*100:.1f}% forgetting (target: <15%)"
        )
