...
from mep.presets import local_ep, sdmep
...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            f"Epoch {epoch}: Avg Loss: {avg_loss:.4f}, Accuracy: {accuracy:.2f}%, Time: {epoch_time:.2f}s"
        )
...
def test(model, device, test_loader):
    ...
    print(
        f"Test set: Average loss: {test_loss:.4f}, Accuracy: {correct}/{len(test_loader.dataset)} ({acc:.2f}%)\n"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dion-thresh", type=int, default=100000, help="Dion threshold"
    )
    parser.add_argument(
        "--optimizer", type=str, default="sdmep", choices=["sdmep", "local_ep", "sgd"]
    )
    parser.add_argument(
        "--dataset", type=str, default="mnist", choices=["mnist", "fashion"]
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="quickly check a single batch",
    )
