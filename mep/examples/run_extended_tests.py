...
def test_extreme_depth(depths=[1000, 2000, 5000, 10000]):
    ...
    for depth in depths:
        ...
        try:
            # Create model
            model = DeepMLP(
                input_dim=64, hidden_dim=64, num_layers=depth, output_dim=10
            ).to(device)
...
def test_mnist_accuracy(depths=[10, 50, 100], epochs=3):
    ...
    for depth in depths:
        ...
        # Create model
        model = DeepMLP(
            input_dim=784, hidden_dim=128, num_layers=depth, output_dim=10
        ).to(device)
...
def test_permuted_mnist_cl(num_tasks=5, epochs=2, ewc_lambda=100):
    ...
    for method in ["ep_ewc", "bp_ewc"]:
        ...
        for task_id in range(num_tasks):
            ...
            model = DeepMLP(
                input_dim=784, hidden_dim=256, num_layers=10, output_dim=10
            ).to(device)
...
def main():
    ...
    if all_results["extreme_depth"]:
        ...
        for r in all_results["extreme_depth"]:
            ...
            print(
                f"   {status} Depth {r['depth']}: {r.get('ep_time_sec', 0):.2f}s, "
    ...
    if all_results["permuted_mnist_cl"]:
        ...
        for method, r in all_results["permuted_mnist_cl"].items():
            if r:
                print(
                    f"   {method}: {r['avg']*100:.1f}% avg, {r['forgetting']*100:.1f}% forgetting"
                )
