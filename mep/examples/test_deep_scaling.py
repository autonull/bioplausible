...
def run_depth_test(
    ...
    try:
        ...
        for epoch in range(epochs):
            if method == "ep":
                train_time, settling_time = train_epoch_ep(
                    model, optimizer, x, y, inspector
                )
        ...
        # Gradient health
        avg_grad_norm = (
            sum(grad_norms_avg) / len(grad_norms_avg) if grad_norms_avg else 0
        )
        ...
        result = ScalingResult(
            ...
            train_time_per_epoch_sec=(
                sum(train_times) / len(train_times) if train_times else 0
            ),
            settling_time_ms=(
                (sum(settling_times) / len(settling_times) * 1000)
                if settling_times
                else 0
            ),
        ...
        print(
            f"    ✓ {method} depth {depth}: {final_acc:.1%} acc, {peak_mem:.1f}MB, {result.train_time_per_epoch_sec:.3f}s/epoch"
        )
...
def analyze_scaling(results: List[ScalingResult]) -> Dict:
    ...
    ep_max_result = next(
        (r for r in ep_results if r.depth == analysis["ep_max_depth"]), None
    )
    bp_max_result = next(
        (r for r in bp_results if r.depth == analysis["bp_max_depth"]), None
    )

    analysis["ep_max_depth_accuracy"] = (
        ep_max_result.final_accuracy if ep_max_result else 0
    )
    analysis["bp_max_depth_accuracy"] = (
        bp_max_result.final_accuracy if bp_max_result else 0
    )
    ...
    if len(ep_results) >= 2:
        ...
        analysis["ep_memory_scaling"] = (ep_mem[-1][1] - ep_mem[0][1]) / (
            ep_mem[-1][0] - ep_mem[0][0]
        )
    else:
    ...
    if len(bp_results) >= 2:
        ...
        analysis["bp_memory_scaling"] = (bp_mem[-1][1] - bp_mem[0][1]) / (
            bp_mem[-1][0] - bp_mem[0][0]
        )
    else:
    ...
    if len(ep_results) >= 2:
        ...
        analysis["ep_time_scaling"] = (ep_time[-1][1] - ep_time[0][1]) / (
            ep_time[-1][0] - ep_time[0][0]
        )
    else:
    ...
    if len(bp_results) >= 2:
        ...
        analysis["bp_time_scaling"] = (bp_time[-1][1] - bp_time[0][1]) / (
            bp_time[-1][0] - ep_time[0][0]
        )
    else:
...
def print_results_table(results: List[ScalingResult]):
    ...
    for r in results:
        ...
        print(
            f"{r.depth:<8} {r.method:<12} {acc:<10} {mem:<12} {time_s:<12} {grad:<12} {status:<10}"
        )
