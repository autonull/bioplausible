...
def print_memory_results(results: List[MemoryComparison]):
    ...
    print("=" * 100)
    print(f"{'Depth':<8} {'Current EP (MB)':<18} {'O(1) EP (MB)':<15} {'Savings (MB)':<15} {'Savings (%)':<12} {'Time Ratio':<10}")
    print("-" * 100)
    
    for r in results:
        time_ratio = r.o1_time_s / r.current_time_s if r.current_time_s > 0 else 0
        print(f"{r.depth:<8} {r.current_ep_mb:<18.2f} {r.o1_ep_mb:<15.2f} {r.savings_mb:<15.2f} {r.savings_percent:>10.1f}% {time_ratio:>8.2f}x")
...
def main():
    ...
    print(f"  Difference: {energy_result.difference:.6e} (tolerance: {energy_result.tolerance:.6e})")
    ...
    settle_result = verify_settle_manual(model, x, y, structure, beta=0.5, steps=30, lr=0.15)
    ...
    training_result, time_current, time_o1 = verify_o1_memory_training(model, x, y, structure)
    ...
    print(f"  Time: Current={time_current:.3f}s, O(1)={time_o1:.3f}s ({time_o1/time_current:.2f}x)")
    ...
    all_correctness_passed = all([energy_result.passed, settle_result.passed, training_result.passed])
    ...
    print(f"\nSuccess Criteria:")
    print(f"  1. Manual settling <1e-5 difference: {'✓' if settle_result.passed else '✗'}")
    print(f"  2. Manual energy <1e-8 difference:   {'✓' if energy_result.passed else '✗'}")
