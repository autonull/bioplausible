#!/usr/bin/env python3
"""
O(1) Memory Prototype: Correctness Verification and Memory Savings

Phase 2: Week 1-2 - Implementation verification

Tests:
1. Manual settling produces identical states (<1e-5 difference)
2. Manual energy produces identical values (<1e-8 difference)
3. Memory savings: 50%+ at depth 500

Run: python examples/verify_o1_memory.py
"""
...
from mep.optimizers import O1MemoryEP, manual_energy_compute, settle_manual
...
def verify_manual_energy(
    ...
    """
    Verify manual energy computation matches current implementation.

    Success criteria: Difference < 1e-8
    """
    energy_fn = EnergyFunction(loss_type="cross_entropy")

    # Current energy (with autograd)
    with torch.enable_grad():
        E_current = energy_fn(model, x, states, structure, target_vec, beta)

    # Manual energy (use_grad=True for fair comparison)
    E_manual = manual_energy_compute(
        model,
        x,
        states,
        structure,
        ...
        use_grad=True,  # Enable grad for comparison
    )

    # Compare
    diff = abs(E_current.item() - E_manual.item())
    tolerance = 1e-8

    passed = diff < tolerance

    return CorrectnessResult(
        test_name="Manual Energy Computation",
        passed=passed,
        difference=diff,
        tolerance=tolerance,
        details=f"E_current={E_current.item():.8f}, E_manual={E_manual.item():.8f}",
    )


def verify_settle_manual(
    model: nn.Module,
    x: torch.Tensor,
    target: torch.Tensor,
    structure: List[Dict[str, Any]],
    ...
    """
    Verify manual settling matches current implementation.
    ...
    states_current = settler.settle(
        model, x, target, beta=beta, energy_fn=energy_fn, structure=structure
    )

    # Manual settling (no autograd overhead)
    states_manual = settle_manual(
        model,
        x,
        target,
        beta=beta,
    ...
    # Compare states
    if len(states_current) != len(states_manual):
        return CorrectnessResult(
            test_name="Manual Settling",
            passed=False,
            difference=float("inf"),
            tolerance=1e-5,
            details=f"State count mismatch: {len(states_current)} vs {len(states_manual)}",
        )
    ...
    return CorrectnessResult(
        ...
        tolerance=tolerance,
        details=f"Mean diff={mean_diff:.6f}, Max diff={max_diff:.6f}",
    )


def verify_o1_memory_training(
    model: nn.Module,
    x: torch.Tensor,
    y: torch.Tensor,
    structure: List[Dict[str, Any]],
    ...
    """
    Verify O(1) memory training produces similar loss.
    ...
    # O(1) memory EP
    model_o1 = DeepMLP(
        x.shape[1], model.hidden_dim, model.num_layers, y.max().item() + 1
    )
    model_o1.load_state_dict(model.state_dict())
    model_o1.to(x.device)

    optimizer_o1 = O1MemoryEP(
        model_o1.parameters(),
        model=model_o1,
        lr=0.01,
        settle_steps=30,
    ...
    result = CorrectnessResult(
        test_name="O(1) Memory Training Step",
        passed=passed,
        difference=diff,
        tolerance=tolerance,
        details=f"Loss current={loss_current:.6f}, Loss O(1)={loss_o1:.6f}",
    )
...
def measure_memory_savings(
    ...
    """
    Measure memory savings of O(1) implementation vs current EP.
    """
...
def print_correctness_results(results: List[CorrectnessResult]):
    ...
    for result in results:
        ...
        print(
            f"  Difference: {result.difference:.6e} (tolerance: {result.tolerance:.6e})"
        )
...
def print_memory_results(results: List[MemoryComparison]):
    ...
    print("\n" + "=" * 100)
    print("MEMORY SAVINGS: O(1) EP vs Current EP")
    print("=" * 100)
    print(
        f"{'Depth':<8} {'Current EP (MB)':<18} {'O(1) EP (MB)':<15} {'Savings (MB)':<15} {'Savings (%)':<12} {'Time Ratio':<10}"
    )
    print("-" * 100)
    ...
    for r in results:
        ...
        print(
            f"{r.depth:<8} {r.current_ep_mb:<18.2f} {r.o1_ep_mb:<15.2f} {r.savings_mb:<15.2f} {r.savings_percent:>10.1f}% {time_ratio:>8.2f}x"
        )
...
def main():
    print("=" * 100)
    print("PHASE 2: WEEK 1-2 - O(1) MEMORY PROTOTYPE VERIFICATION")
    print("=" * 100)
    ...
    # Get settled states for energy/settling tests
    settler = Settler(steps=30, lr=0.15)
    energy_fn = EnergyFunction(loss_type="cross_entropy")

    print("\n" + "=" * 100)
    print("TEST 1: MANUAL ENERGY CORRECTNESS")
    print("=" * 100)

    states_free = settler.settle(
        model, x, None, beta=0.0, energy_fn=energy_fn, structure=structure
    )
    energy_result = verify_manual_energy(model, x, states_free, structure, None, 0.0)

    print(f"\n{energy_result.test_name}")
    print(f"  Status: {'✓ PASS' if energy_result.passed else '✗ FAIL'}")
    print(
        f"  Difference: {energy_result.difference:.6e} (tolerance: {energy_result.tolerance:.6e})"
    )
    print(f"  Details: {energy_result.details}")

    print("\n" + "=" * 100)
    print("TEST 2: MANUAL SETTLING CORRECTNESS")
    print("=" * 100)

    settle_result = verify_settle_manual(
        model, x, y, structure, beta=0.5, steps=30, lr=0.15
    )

    print(f"\n{settle_result.test_name}")
    print(f"  Status: {'✓ PASS' if settle_result.passed else '✗ FAIL'}")
    print(
        f"  Difference: {settle_result.difference:.6e} (tolerance: {settle_result.tolerance:.6e})"
    )
    print(f"  Details: {settle_result.details}")

    print("\n" + "=" * 100)
    print("TEST 3: O(1) MEMORY TRAINING CORRECTNESS")
    print("=" * 100)

    training_result, time_current, time_o1 = verify_o1_memory_training(
        model, x, y, structure
    )
    ...
    print(
        f"  Difference: {training_result.difference:.6e} (tolerance: {training_result.tolerance:.6e})"
    )
    print(f"  Details: {training_result.details}")
    print(
        f"  Time: Current={time_current:.3f}s, O(1)={time_o1:.3f}s ({time_o1/time_current:.2f}x)"
    )

    # Print all correctness results
    print_correctness_results([energy_result, settle_result, training_result])
    ...
    for d in depths:
        print(f"\nMeasuring depth {d}...")
        result = measure_memory_savings(depth=d)
        memory_results.append(result)
        print(
            f"  Current EP: {result.current_ep_mb:.2f} MB ({result.current_time_s:.3f}s)"
        )
        print(f"  O(1) EP:    {result.o1_ep_mb:.2f} MB ({result.o1_time_s:.3f}s)")
        print(
            f"  Savings:    {result.savings_mb:.2f} MB ({result.savings_percent:.1f}%)"
        )
    ...
    all_correctness_passed = all(
        [energy_result.passed, settle_result.passed, training_result.passed]
    )
    ...
    print(
        f"  2. Manual energy <1e-8 difference:   {'✓' if energy_result.passed else '✗'}"
    )
