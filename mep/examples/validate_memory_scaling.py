...
def run_scaling_experiment(
    ...
    for depth in depths:
        ...
        if use_checkpointing:
            model_bp = CheckpointedDeepMLP(input_dim, hidden_dim, depth, output_dim).to(
                device
            )
        else:
...
def analyze_scaling(results: List[MemoryMeasurement]) -> Dict:
    ...
    savings = (
        1 - max_depth_ep.activation_memory_mb / max_depth_bp.activation_memory_mb
    ) * 100
...
def print_results_table(results: List[MemoryMeasurement]):
    ...
    print("=" * 90)
    print(
        f"{'Depth':<8} {'Method':<12} {'Checkpoint':<12} {'Activation MB':<15} {'Total MB':<12} {'Time (s)':<10} {'Status':<8}"
    )
    print("-" * 90)
    ...
    for r in results:
        ...
        print(
            f"{r.depth:<8} {r.method:<12} {checkpoint:<12} {act_mem:<15} {total_mem:<12} {time_s:<10} {status:<8}"
        )
...
def plot_results(results: List[MemoryMeasurement], save_path: Optional[str] = None):
    ...
    ax1 = axes[0]
    ax1.plot(
        [r.depth for r in bp_results],
        [r.activation_memory_mb for r in bp_results],
        "o-",
        label="Backprop",
        linewidth=2,
        markersize=8,
    ...
    ax1.plot(
        [r.depth for r in ep_results],
        [r.activation_memory_mb for r in ep_results],
        "s-",
        label="EP",
        linewidth=2,
        markersize=8,
    ...
    for depth in depths:
        bp_mem = next(
            (r.activation_memory_mb for r in bp_results if r.depth == depth), None
        )
        ep_mem = next(
            (r.activation_memory_mb for r in ep_results if r.depth == depth), None
        )
    ...
    ax2.bar(
        range(len(depths)), [s if s else 0 for s in savings], color="green", alpha=0.6
    )
...
def main():
    ...
    print("=" * 90)
    print(
        f"Backprop scaling: {analysis.get('bp_scaling_mb_per_layer', 0):.4f} MB/layer"
    )
    print(
        f"EP scaling:       {analysis.get('ep_scaling_mb_per_layer', 0):.4f} MB/layer"
    )
    print(
        f"Memory savings at max depth: {analysis.get('memory_savings_at_max_depth_pct', 0):.1f}%"
    )
    ...
    if bp_scaling > ep_scaling * 2:
        print(
            f"  ✅ EP shows O(1) scaling (backprop scales {bp_scaling/ep_scaling:.1f}x faster)"
        )
