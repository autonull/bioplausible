...
def run_depth_experiment(
    ...
    bp_result = MemoryResult(
        depth=depth, method="backprop", peak_memory_mb=0, success=False
    )
...
def main():
    ...
    for depth in depths:
        ...
        bp_status = (
            f"{bp_result.peak_memory_mb:.1f} MB"
            if bp_result.success
            else bp_result.error
        )
        ep_status = (
            f"{ep_result.peak_memory_mb:.1f} MB"
            if ep_result.success
            else ep_result.error
        )
        ...
        print(
            f"{depth:<8} {'Backprop':<12} {bp_status:<15} {bp_time:<12} {'✓' if bp_result.success else '✗':<10}"
        )
        print(
            f"{depth:<8} {'EP':<12} {ep_status:<15} {ep_time:<12} {'✓' if ep_result.success else '✗':<10}"
        )
        print()
    ...
    for bp_result, ep_result in results:
        if bp_result.success and ep_result.success:
            ...
            print(
                f"{bp_result.depth:<8} {bp_result.peak_memory_mb:<12.1f} {ep_result.peak_memory_mb:<12.1f} {savings:>6.1f}%"
            )
    ...
    if bp_failed_at and not ep_failed_at:
        ...
        print(
            f"  ⚠️  EP failed at greater depth than backprop ({ep_failed_at} vs {bp_failed_at})"
        )
