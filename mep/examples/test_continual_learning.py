...
def generate_task_data(
    task_id, num_samples=200, input_dim=50, num_classes=5, seed=None
):
...
def run_cl_benchmark(num_tasks=3, epochs=2, batch_size=32, ewc_lambda=100):
    ...
    for method in methods:
        ...
        model = MLP(input_dim=input_dim, hidden_dim=64, output_dim=num_classes).to(
            device
        )
        ...
        for task_id in range(num_tasks):
            # Generate task data
            X_train, y_train = generate_task_data(
                task_id, num_samples=200, seed=42 + task_id
            )
            X_test, y_test = generate_task_data(
                task_id, num_samples=50, seed=142 + task_id
            )
            ...
            for epoch in range(epochs):
                for X, y in train_loader:
                    ...
                    if use_ep:
                        ...
                        if (
                            use_ewc
                            and hasattr(opt, "ewc_state")
                            and opt.ewc_state is not None
                        ):
            ...
            for prev_task in range(task_id + 1):
                X_prev, y_prev = generate_task_data(
                    prev_task, num_samples=50, seed=142 + prev_task
                )
            ...
            print(
                f"  Task {task_id}: Train acc = {current_accs[-1]*100:.1f}%, "
