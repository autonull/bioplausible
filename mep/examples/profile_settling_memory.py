...
def measure_settling_memory(
    ...
    states_o1 = settle_manual_o1(
        model_o1, x, None, beta=0.0, structure=structure_o1, steps=30, lr=0.15
    )
    ...
    return {
        ...
        "settling_savings_percent": (
            (settling_mem_current - settling_mem_o1) / settling_mem_current * 100
            if settling_mem_current > 0
            else 0
        ),
    }
...
def measure_contrast_memory(
    ...
    states_nudged = settler.settle(
        model, x, None, beta=0.5, energy_fn=energy_fn, structure=structure
    )
    ...
    states_free_o1 = settler.settle(
        model_o1, x, None, beta=0.0, energy_fn=energy_fn, structure=structure_o1
    )
    states_nudged_o1 = settler.settle(
        model_o1, x, None, beta=0.5, energy_fn=energy_fn, structure=structure_o1
    )

    reset_memory()
    torch.cuda.reset_peak_memory_stats()

    E_free_o1 = energy_from_states_minimal(
        model_o1, x, states_free_o1, structure_o1, None, 0.0, loss_type="cross_entropy"
    )
    E_nudged_o1 = energy_from_states_minimal(
        model_o1,
        x,
        states_nudged_o1,
        structure_o1,
    ...
    return {
        ...
        "contrast_savings_percent": (
            (contrast_mem_current - contrast_mem_o1) / contrast_mem_current * 100
            if contrast_mem_current > 0
            else 0
        ),
    }
...
def measure_full_step_memory(
    ...
    return {
        ...
        "full_savings_percent": (
            (mem_current - mem_o1) / mem_current * 100 if mem_current > 0 else 0
        ),
    }
...
def main():
    ...
    for d in depths:
        ...
        print(
            f"  Depth {d}: Current={result['current_settling_mb']:.2f}MB, O(1)={result['o1_settling_mb']:.2f}MB, Savings={result['settling_savings_percent']:.1f}%"
        )
    ...
    for d in depths:
        ...
        print(
            f"  Depth {d}: Current={result['current_contrast_mb']:.2f}MB, O(1)={result['o1_contrast_mb']:.2f}MB, Savings={result['contrast_savings_percent']:.1f}%"
        )
    ...
    for d in depths:
        ...
        print(
            f"  Depth {d}: Current={result['current_full_mb']:.2f}MB, O(1)={result['o1_full_mb']:.2f}MB, Savings={result['full_savings_percent']:.1f}%"
        )
