...
parser.add_argument(
    "--seed-base", type=int, default=42, help="Base random seed for reproducibility"
)
...
for model_name in models_to_test:
    ...
    def objective(trial):
        ...
        config = create_optuna_space(trial, model_name, evaluation_config=eval_config)

        # Add seed for reproducibility
        config["seed"] = SEED_BASE + trial.number

        print(
            f"    lr={config['lr']:.6f}, hidden={config['hidden_dim']}, layers={config['num_layers']}",
            end="",
        )
        if "beta" in config:
        ...
        if metrics:
            ...
            print(
                f"    → acc={accuracy:.4f}, params={param_count:.2f}M, time={iter_time:.4f}s"
            )
...
for rank, entry in enumerate(comparison, 1):
    print(
        f"{rank:<5} {entry['model']:<35} {entry['accuracy']:<10.4f} {entry['params']:<12.2f} {entry['time']:<10.4f} {entry['trial_num']}"
    )
...
for model_name, result in results.items():
    ...
    completed_trials = [
        t for t in all_trials[model_name] if t.state == optuna.trial.TrialState.COMPLETE
    ]
    ...
    if "beta" in top_25_pct[0].params:
        ...
        all_beta_mean = sum(t.params["beta"] for t in completed_trials) / len(
            completed_trials
        )
...
for model_name, result in results.items():
    if "time" in result:
        ...
        pruned = sum(
            1
            for t in all_trials[model_name]
