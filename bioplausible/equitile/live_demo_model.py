...
class FastLMConfig(LMEquiTileConfig):
    ...
    sparsity_weight: float = (
        0.5  # L1 regularization to encourage sparse tile activation
    )
    sparsity_threshold: float = 0.1  # Threshold for considering a tile "active"
    importance_lr: float = (
        0.1  # Separate learning rate for tile importance (higher than weight lr)
    )
    ...
    competitive_k_ratio: float = (
        0.5  # Ratio of tiles to keep active in competitive mode
    )
...
class DemoEquiTileLayer(OptimizedEquiTileTransformerLayer):
    ...
    def update_activity_ema(self, current_activity):
        ...
        with torch.no_grad():
            self.activity_ema = (
                self.ema_decay * self.activity_ema
                + (1 - self.ema_decay) * current_activity
            )
...
class FastLMEquiTile(OptimizedLMEquiTile):
        ...
        if "learning_rate" in params:
            for g in self.optimizer.param_groups:
                if "tile_importance" not in g.get(
                    "name", ""
                ) and "gate_logits" not in g.get("name", ""):
    ...
    def get_tile_details(
        ...
        rng = np.random.default_rng(
            seed=layer_idx * 1000 + tile_idx + self._step_counter
        )
        neuron_acts = rng.exponential(
            scale=max(0.01, avg_act), size=self.config.neurons_per_tile
        )
    ...
    def training_step(
        ...
        if input_ids is None:
            if self.dataset is not None:
                ...
                indices = torch.randint(
                    0, len(self.dataset), (self.fast_config.batch_size,)
                )
                batch_x = []
                for idx in indices:
                ...
                input_ids = torch.randint(
                    0,
                    self._vocab_size,
                    (self.fast_config.batch_size, self._seq_len),
                    device=device,
                )
        ...
        for layer in self.layers:
            ...
            sparsity_loss = (
                sparsity_loss
                + self.fast_config.sparsity_weight * (importance * gate_prob).sum()
            )
        ...
        for layer in self.layers:
            ...
            decay_loss = (
                decay_loss
                + self.fast_config.importance_decay
                * ((importance_sigmoid - 0.5) ** 2).sum()
            )
        ...
        if self.fast_config.use_competitive:
            for layer in self.layers:
                k = int(
                    self.config.tiles_per_layer * self.fast_config.competitive_k_ratio
                )
                # Keep only top-k tiles by importance
                top_k_values, top_k_indices = torch.topk(
                    layer.tile_importance.squeeze(), k
                )
                ...
                layer.tile_importance.data = (
                    layer.tile_importance * mask
                    + (layer.tile_importance * mask - layer.tile_importance).detach()
                )
        ...
        dynamic_sparsity = self.fast_config.sparsity_weight * (
            1.0 + 0.1 * math.sin(self._step_counter * 0.1)
        )
        ...
        for layer in self.layers:
            ...
            gate_prob = torch.sigmoid(layer.gate_logits)
            sparsity_loss = (
                sparsity_loss + dynamic_sparsity * (importance * gate_prob).sum()
            )
        ...
        for layer in self.layers:
            if (
                hasattr(layer, "tile_importance")
                and layer.tile_importance.grad is not None
            ):
        ...
        with torch.no_grad():
            for layer in self.layers:
                if (
                    ...
                    contrib = (
                        (grad.abs() * torch.sigmoid(layer.tile_importance))
                        .cpu()
                        .numpy()
                    )
        ...
        if self._step_counter == 0 or self._step_counter % 10 == 0:
            ...
            if self.dataset:
                ...
                prefix = (
                    ...
                    else f"Step {self._step_counter}"
                )
                ...
                prefix = (
                    "Initial"
                    if self._step_counter == 0
                    else f"Step {self._step_counter} (Random)"
                )
                generated_text = f"{prefix}:\n" + " ".join(
                    str(t.item()) for t in gen_ids[0]
                )
        else:
            generated_text = ""
