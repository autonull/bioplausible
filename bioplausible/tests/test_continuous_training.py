...
from bioplausible.scientist.training_dynamics import (
    TrainingTrajectory, TrainingCheckpoint, ContinuousTrainingSchedule
)
...
class TestContinuousTraining(unittest.TestCase):
    ...
    def test_convergence_detection(self):
        ...
        checkpoints = [
            TrainingCheckpoint(epoch=1, val_acc=0.5, train_acc=0.6, train_loss=1.0, val_loss=1.0),
            TrainingCheckpoint(epoch=2, val_acc=0.6, train_acc=0.7, train_loss=0.8, val_loss=0.8),
            TrainingCheckpoint(epoch=5, val_acc=0.7, train_acc=0.8, train_loss=0.6, val_loss=0.6),
            TrainingCheckpoint(epoch=10, val_acc=0.705, train_acc=0.8, train_loss=0.5, val_loss=0.6), # < 1% improvement
            TrainingCheckpoint(epoch=20, val_acc=0.708, train_acc=0.8, train_loss=0.4, val_loss=0.6),
        ]
        
        traj = TrainingTrajectory(
            trial_id=1, model_name="test", task_name="test", config={}, checkpoints=checkpoints
        )
    ...
    def test_storage_integration(self):
        ...
        try:
            ...
            traj = TrainingTrajectory(
                ...
                config=config,
                checkpoints=[
                    TrainingCheckpoint(epoch=1, val_acc=0.5, train_acc=0.6, train_loss=1.0, val_loss=1.1),
                    TrainingCheckpoint(epoch=5, val_acc=0.7, train_acc=0.8, train_loss=0.5, val_loss=0.6)
                ]
            )
            traj.converged = True
            ...
            cursor.execute("SELECT * FROM training_trajectories WHERE trial_id=?", (trial_id,))
            row_traj = cursor.fetchone()
            self.assertIsNotNone(row_traj)
            self.assertEqual(row_traj["task_name"], "test_task")
            
            cursor.execute("SELECT * FROM training_checkpoints WHERE trajectory_id=?", (row_traj["id"],))
    ...
    def test_schedule_execution_logic(self):
        ...
        traj = schedule.train_with_checkpoints(
            trainer, trial_id=999, model_name="m", task_name="t", config={}, on_epoch_end=on_epoch_end
        )
