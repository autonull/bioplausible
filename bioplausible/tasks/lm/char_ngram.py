...
class CharNGramTask(BaseTask):
    ...
    def __init__(self, name="char_ngram", device="cpu", quick_mode=False, vocab_size=27, context_len=3):
    ...
    def get_batch(self, split="train", batch_size=32) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for s in starts:
            ...
            seq = (torch.arange(s.item(), s.item() + self.context_len + 1)) % self.vocab_size
            x_list.append(seq[:-1])
            y_list.append(seq[-1])
