...
from .equitile import EquiTileConfig, TileState, EdgeParams, TileGraph

# Explicitly add inherited fields to EnhancedEquiTileConfig for dataclass compatibility
# This is needed because dataclass inheritance can be tricky with multiple inheritance
EQUITILE_CONFIG_FIELDS = {
    'neurons_per_tile': 64,
    'num_layers': 4,
    'tiles_per_layer': 4,
    'learning_rate': 0.01,
    ...
    'beta': 0.1,
    'beta_anneal': 1.0,
    'inference_steps_free': None,
    'inference_steps_nudged': None,
...
class EnhancedEquiTile(BioModel):
    ...
    -------------
    >>> # Minimal (baseline)
    >>> config = EnhancedEquiTileConfig.preset_minimal()
    >>> model = EnhancedEquiTile(config, ...)
    
    >>> # Vision-optimized
    >>> config = EnhancedEquiTileConfig.preset_vision()
    >>> model = EnhancedEquiTile(config, ...)
    
    >>> # Custom ablation
    >>> config = EnhancedEquiTileConfig(
    ...     use_layer_norm=True,
    ...     use_residual_errors=False,  # Disable for ablation
    ...     per_tile_lr=True,
    ... )
    """
        ...
        if self.config.use_layer_norm:
            for tile in self.graph.all_tiles:
                if not tile.is_input:
                    self.layer_norms[str(tile.id)] = LayerNormTile(
