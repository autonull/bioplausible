...
class DTPLayer(nn.Module):
    def __init__(self, in_features, out_features):
        super().__init__()
        self.forward_net = nn.Sequential(nn.Linear(in_features, out_features), nn.Tanh())
        self.inverse_net = nn.Sequential(nn.Linear(out_features, in_features), nn.Tanh())
    ...
    def train_step(self, x: torch.Tensor, y: torch.Tensor) -> Dict[str, float]:
        ...
        for i in reversed(range(len(self.layers))):
            ...
            if i > 0:
                ...
                with torch.no_grad():
                    t_prev = h_prev - layer.inverse_net(h_curr) + layer.inverse_net(t_curr)
