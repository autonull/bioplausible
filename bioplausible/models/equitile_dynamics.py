...
if TYPE_CHECKING:
    from .equitile import EquiTile, TileState, EdgeParams
...
class TileGrowthConfig:
    """Configuration for tile growth/pruning."""
    # Growth
    growth_enabled: bool = True
    growth_threshold: float = 0.5  # Add tile if error > threshold
    growth_cooldown: int = 100  # Steps between growth
    max_tiles: int = 100
    ...
    prune_cooldown: int = 200  # Steps between pruning
    min_tiles: int = 2
    min_tiles_per_layer: int = 1

    # Merging
    merge_enabled: bool = False
    merge_threshold: float = 0.8  # Cosine similarity for merging
    merge_cooldown: int = 500

    # Splitting
    split_enabled: bool = False
    split_threshold: float = 1.0  # Error threshold for splitting
    split_cooldown: int = 300

    # General
    error_ema_decay: float = 0.95
    min_age_for_modify: int = 50  # Tiles must be this old to modify
...
class DynamicEquiTileConfig:
    """Configuration for dynamic EquiTile."""
