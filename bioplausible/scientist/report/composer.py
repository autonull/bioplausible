...
class ReportComposer:
    ...
    def _generate_visualizations(self, df: pd.DataFrame, manifest: Dict):
        ...
        for d in data:
            if d.get("iteration_time") is None: d["iteration_time"] = 0
            if d.get("accuracy") is None: d["accuracy"] = 0.0
            if d.get("param_count") is None: d["param_count"] = 0
        ...
        manifest["images"].append({"title": "Pareto Efficiency Frontier", "path": Path(path).name})

        # 2. Convergence Speed
        path = self.visualizer.plot_convergence_speed(data)
        if path:  # Only if data available
            manifest["images"].append({"title": "Convergence Speed", "path": Path(path).name})
        
        # 3. Tier Progress
        path = self.visualizer.plot_tier_progress(data)
        manifest["images"].append({"title": "Progress by Tier", "path": Path(path).name})
        ...
        for task in tasks:
            ...
            if path:
                manifest["images"].append({"title": f"Leaderboard (Accuracy): {task}", "path": Path(path).name})
            
            # Efficiency Leaderboard (Accuracy / Params)
            path = self.visualizer.plot_leaderboard(data, task, metric="efficiency")
            if path:
                manifest["images"].append({"title": f"Leaderboard (Efficiency): {task}", "path": Path(path).name})
    ...
    def _generate_significance_matrix(self, df: pd.DataFrame, data: List[Dict], manifest: Dict):
        ...
        try:
            ...
            manifest["images"].append({"title": "Statistical Significance Matrix", "path": Path(path).name})
