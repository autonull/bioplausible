...
class KnowledgebaseMetamodel:
            ...
            for _, row in raw_data.iterrows():
                try:
                    ...
                    record = {
                        ...
                        "max_steps": cfg.get("extra", {}).get("max_steps", 0.0) if "extra" in cfg else cfg.get("max_steps", 0.0)
                    }
    ...
    def extract_symbolic_rules(self, target_metric: str = "outcome", focus_model: str = "eqprop_mlp") -> List[str]:
        """
        ...
        if len(np.unique(Y)) < 2:
             return [f"For {focus_model}: Outcome is uniform; cannot extract discriminative rules."]
        ...
        formatted_rules = []
        formatted_rules.append(f"Heuristics for {focus_model} avoiding Settling Divergence:")
        
        for line in rules_text.split('\n'):
            if line.strip():
                formatted_rules.append("  " + line.strip().replace("value: [0.0]", "--> Success").replace("value: [1.0]", "--> Divergence"))
