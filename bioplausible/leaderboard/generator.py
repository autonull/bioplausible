...
class LeaderboardGenerator:
    ...
    def export_markdown(self, output_path: str = "reports/leaderboard.md"):
        ...
        with open(output_path, 'w') as f:
            ...
            f.write("A quantitative comparison of alternative learning rules against standard backpropagation benchmarks.\n\n")
            ...
            acc_cols = ["algorithm", "task", "val_accuracy", "requires_backward", "mean_acc", "std_acc"]
            self._write_markdown_table(f, acc_view, acc_cols)
            
            # --- 2. Energy Efficiency ---
            f.write("## ⚡ Best Energy Efficiency\n\n")
            f.write("Sorted by lowest energy proxy footprint against parameter cost.\n\n")
            erg_view = self.df.sort_values(by="energy_proxy", ascending=True)
            erg_cols = ["algorithm", "task", "energy_proxy", "backward_flops", "param_count", "val_accuracy"]
            ...
            hw_cols = ["algorithm", "task", "peak_memory_mb", "wall_time_s", "param_count"]
            self._write_markdown_table(f, hw_view, hw_cols)

    def _write_markdown_table(self, file_handle, dataframe: pd.DataFrame, columns: List[str]):
