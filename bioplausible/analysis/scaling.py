...
def plot_scaling_curves(
    results_df: pd.DataFrame, metric: str = "val_loss"
) -> plt.Figure:
    """
    Per-algorithm scaling curves with power-law fits.
    """
    if (
        "param_count" not in results_df.columns
        or "model" not in results_df.columns
        or metric not in results_df.columns
    ):
        raise ValueError(
            f"DataFrame must contain 'model', 'param_count', and '{metric}' columns."
        )
    ...
    ax.set_title(
        f"Scaling Laws: {metric.replace('_', ' ').title()} vs. Parameter Count"
    )
