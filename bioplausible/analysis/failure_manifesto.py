...
from bioplausible.scientist.failure_tracker import FailureTracker, FailureCategory
...
class FailureManifestoGenerator:
    ...
    def generate(self, output_path: str = "reports/failure_manifesto.md"):
        ...
        with open(output_path, "w") as f:
            ...
            f.write("This document tracks the explicit failure modes encountered across different bioplausible algorithms.\n\n")
            ...
            if not analysis.get("recommendations"):
                f.write("No critical failure patterns detected requiring immediate intervention.\n")
            else:
                for rec in analysis["recommendations"]:
                    sev = rec.get('severity', 'info')
                    f.write(f"### [Severity: {sev.upper()}] {rec.get('issue', 'Unknown Issue')}\n")
                    f.write(f"- **Recommendation**: {rec.get('suggestion')}\n")
                    if 'affected_models' in rec:
                        f.write(f"- **Affected Models**: {', '.join(rec['affected_models'])}\n")
                    if 'details' in rec:
