...
class BenchmarkVisualizer:
    ...
    def plot_optimizer_comparison_with_stats(
        ...
        ax.set_title(
            "Optimizer Comparison: Final Test Accuracy\n(with 95% CI error bars)",
            fontsize=14,
            fontweight="bold",
        )
        ax.set_xticks(x_pos)
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(self.output_dir, "optimizer_comparison_stats.png"),
            bbox_inches="tight",
        )
        plt.close()
    ...
    def plot_training_curves_all(self, results: dict[str, Any]) -> None:
        ...
        plt.tight_layout()
        plt.savefig(
            os.path.join(self.output_dir, "training_curves_all.png"),
            bbox_inches="tight",
        )
        plt.close()
    ...
    def plot_test_accuracy_comparison(self, results: dict[str, Any]) -> None:
        ...
        for opt_name in results["optimizers"].keys():
            color = color_map.get(opt_name.lower(), "#666666")
            handles.append(
                plt.Line2D([0], [0], color=color, linewidth=2, label=opt_name.upper())
            )

        ax.legend(handles=handles, loc="lower right", fontsize=9)
        plt.tight_layout()
        plt.savefig(
            os.path.join(self.output_dir, "test_accuracy_comparison.png"),
            bbox_inches="tight",
        )
        plt.close()
    ...
    def plot_time_analysis(self, results: dict[str, Any]) -> None:
        ...
        if optimizer_names:
            ...
            bar_colors = [
                color_map.get(name.lower(), "#666666") for name in optimizer_names
            ]
            ...
            axes[1].set_title(
                "Computational Cost per Step", fontsize=14, fontweight="bold"
            )
            ...
            plt.tight_layout()
            plt.savefig(
                os.path.join(self.output_dir, "time_analysis.png"), bbox_inches="tight"
            )
            plt.close()
    ...
    def generate_summary_report(self, results: dict[str, Any]) -> str:
        ...
        report.append(
            f"**Target Time per Trial:** {results['metadata']['target_time_per_trial']}s\n"
        )
        ...
        report.append("\n## Results Summary\n")
        report.append(
            "| Optimizer | Mean Acc (%) | Std (%) | Min (%) | Max (%) | vs SGD |"
        )
        report.append(
            "|-----------|--------------|---------|---------|---------|--------|"
        )
        ...
        for opt_name, opt_data in results["optimizers"].items():
            ...
            report.append(
                f"| {self._format_optimizer_name(opt_name)} | {mean_acc:.2f} | {std_acc:.2f} | {min_acc:.2f} | {max_acc:.2f} | {comparison} |"
            )
