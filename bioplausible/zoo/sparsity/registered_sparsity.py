"""
Registered sparsity methods in the unified Zoo.

Sparsity/pruning methods registered with @register_sparsity
for AutoScientist discovery and composition.
"""
...
@register_sparsity(
    ...
    tags=["sparsity", "pruning", "top-k", "tile-eq"],
    description=(
        "Top-K Scheduling: selectively updates"
        " top-k most active connections per tile"
    ),
)
class _RegisteredTopKPruning(TopKScheduling):
    pass
...
@register_sparsity(
    ...
    description=(
        "Activity-driven sparsity: prune connections"
        " with low activity; Hebbian-inspired"
    ),
)
...
class RandomPruning:
    ...
    def step(self):
        ...
        for param in self.model.parameters():
            if param.ndim >= 2:
                with torch.no_grad():
                    mask = torch.rand_like(param) > self.prune_fraction
                    param.data.mul_(mask)
