...
def parse_args():
    ...
    parser.add_argument(
        "--model", type=str, default="EquiTile", help="Model name to load"
    )
    parser.add_argument("--task", type=str, default="lm", help="Task type (lm, vision)")
    parser.add_argument("--num-layers", type=int, default=4, help="Number of layers")
    parser.add_argument(
        "--tiles-per-layer", type=int, default=16, help="Hidden units/tiles per layer"
    )
    parser.add_argument(
        "--neurons-per-tile", type=int, default=32, help="Neurons per tile (or unit)"
    )
    parser.add_argument("--batch-size", type=int, default=16, help="Batch size")
    parser.add_argument(
        "--seq-len", type=int, default=64, help="Maximum sequence length"
    )
    parser.add_argument(
        "--dataset", type=str, default="Tiny Shakespeare", help="Dataset name"
    )
