...
def benchmark_scaling():
    ...
    for config in configs:
        n_tiles = (
            config["tiles_per_layer"] * (config["num_layers"] - 2) + 2
        )  # +2 for input/output
...
def benchmark_pc_vs_ep():
    ...
    return {
        "pc": {
            "memory_mb": mem_pc,
            "time_s": time_pc,
            "throughput": n_samples * 5 / time_pc,
        },
        "ep": {
            "memory_mb": mem_ep,
            "time_s": time_ep,
            "throughput": n_samples * 5 / time_ep,
        },
    }
...
def benchmark_inference_steps():
    ...
    for steps in steps_configs:
        ...
        print(
            f"Steps: {steps:2d} | Time: {elapsed:6.2f}s | Throughput: {throughput:7.1f} samples/s"
        )
...
def benchmark_memory_efficiency():
    ...
    for config in configs:
        ...
        print(
            f"Config: {config['neurons_per_tile']} neurons/tile, "
