...
class ResultAnalyzer:
    ...
    def compare_optimizers(
        ...
        """
        values_a = [
            getattr(r, metric) for r in self.results if r.optimizer_name == optimizer_a
        ]
        values_b = [
            getattr(r, metric) for r in self.results if r.optimizer_name == optimizer_b
        ]
    ...
    def get_optimizer_ranking(
        ...
        rankings = [(opt, np.mean(values)) for opt, values in optimizer_metrics.items()]
    ...
    def get_model_ranking(  # noqa: E704
        ...
        rankings = [(model, np.mean(values)) for model, values in model_metrics.items()]
