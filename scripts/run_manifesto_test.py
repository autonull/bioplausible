...
def main():
    ...
    failures = [
        ...
        FailureRecord(
            ...
            tier="baseline",
            trial_id=1,
            failure_type=FailureCategory.SETTLING_DIVERGENCE.value,
            failure_epoch=5,
            failure_batch=100,
            config={"lr": 0.05},
            last_metrics={},
            stack_trace="State failed to settle",
        ),
        FailureRecord(
            timestamp=datetime.now().isoformat(),
            model_name="eqprop_mlp",
        ...
        FailureRecord(
            ...
            tier="baseline",
            trial_id=3,
            failure_type=FailureCategory.GOODNESS_COLLAPSE.value,
            failure_epoch=1,
            failure_batch=10,
            config={"lr": 0.001},
            last_metrics={},
            stack_trace="Goodness = 0",
        ),
        ...
        FailureRecord(
            ...
            tier="scaling",
            trial_id=4,
            failure_type=FailureCategory.MEMORY_OOM.value,
            failure_epoch=1,
            failure_batch=0,
            config={"num_layers": 128},
            last_metrics={},
            stack_trace="CUDA OOM",
        ),
        # STDP Silencing
        FailureRecord(
            timestamp=datetime.now().isoformat(),
            model_name="spiking_stdp",
            task_name="mnist",
            tier="baseline",
            trial_id=5,
            failure_type=FailureCategory.SPIKE_SILENCING.value,
            failure_epoch=3,
            failure_batch=500,
            config={"lr": 1e-4},
            last_metrics={},
            stack_trace="0 spikes detected",
        ),
        ...
        FailureRecord(
            ...
            tier="baseline",
            trial_id=6,
            failure_type=FailureCategory.GRADIENT_EXPLOSION.value,
            failure_epoch=0,
            failure_batch=5,
            config={"lr": 1.0},
            last_metrics={},
