...
class GradientHealthPanel(QGroupBox):
    ...
    def __init__(self, num_layers=4):
        ...
        self.setStyleSheet("QGroupBox { font-weight: bold; color: #ff6600; border: 1px solid #555; }")
...
class SparsityTimelinePanel(QGroupBox):
    ...
    def __init__(self, max_steps=200):
        ...
        self.setStyleSheet("QGroupBox { font-weight: bold; color: #00ccff; border: 1px solid #555; }")
        ...
        self.target_line = self.plot.plot(pen=pg.mkPen('#ff00ff', style=Qt.PenStyle.DashLine))
    ...
    def update_sparsity(self, sparsity_pct, target_pct=30):
        ...
        if len(self.history) > 10:
            ...
            older = np.mean(self.history[-20:-10]) if len(self.history) > 20 else self.history[0]
...
class ActivationDistributionPanel(QGroupBox):
    ...
    def __init__(self):
        ...
        self.setStyleSheet("QGroupBox { font-weight: bold; color: #00ff88; border: 1px solid #555; }")
...
class ModelHealthSummary(QGroupBox):
    ...
    def __init__(self):
        ...
        self.setStyleSheet("QGroupBox { font-weight: bold; color: #00ff00; border: 2px solid #555; }")
    ...
    def update(self, grad_stats, train_acc, test_acc, sparsity, loss_history):
        ...
        if train_acc - test_acc > 10:
            self.overfit_status.setText(f"Overfitting: ⚠️ Gap={train_acc-test_acc:.1f}%")
            self.overfit_status.setStyleSheet("color: #ff6600;")
        else:
            self.overfit_status.setText(f"Overfitting: ✓ OK (gap={abs(train_acc-test_acc):.1f}%)")
        ...
        if len(loss_history) > 20:
            ...
            if len(older) > 0:
                ...
                if change < 1:
                    ...
                    self.convergence_status.setText(f"Convergence: ~ Learning ({change:.1f}%/20 steps)")
                    self.convergence_status.setStyleSheet("color: #ffff00;")
                else:
                    self.convergence_status.setText(f"Convergence: ⚠️ Diverging ({abs(change):.1f}%)")
        ...
        if len(loss_history) > 20 and change < 0:
            suggestions.append("Reduce learning rate, check for catastrophic forgetting")
...
class AnomalyDetector:
    ...
    def check(self, step, loss, sparsity, grad_norms):
        ...
        if len(self.loss_history) > 5:
            ...
            older = np.mean(self.loss_history[-10:-5]) if len(self.loss_history) > 10 else self.loss_history[0]
