...
class DashboardPanel(QWidget):
    ...
    def init_ui(self):
        ...
        self.loss_data = []
        self.ppl_data = []
        self.loss_curve = self.loss_plot.plot(pen=pg.mkPen('#ff00ff', width=2), name='Loss')
        self.ppl_curve = self.loss_plot.plot(pen=pg.mkPen('#ffff00', width=2), name='Perplexity')
        ...
        self.train_acc_data = []
        self.test_acc_data = []
        self.train_acc_curve = self.acc_plot.plot(pen=pg.mkPen('#00ff88', width=2), name='Train')
        self.test_acc_curve = self.acc_plot.plot(pen=pg.mkPen('#00ccff', width=2), name='Test')
        ...
        self.tile_loss_bars = pg.BarGraphItem(x=[], height=[], width=0.8, brush='#ff6600')
    ...
    def update_metrics(self, loss, tokens_per_sec, sparsity, train_acc=None, test_acc=None, perplexity=None, tile_losses=None):
        # Loss and Perplexity
        ...
        if perplexity is not None:
            ...
            if len(self.ppl_data) > 300: self.ppl_data.pop(0)
            self.ppl_curve.setData(self.ppl_data)
        
        self.loss_plot.setTitle(f"Loss: {loss:.4f} | Perplexity: {perplexity:.1f}" if perplexity else f"Loss: {loss:.4f}")

        # Accuracy
        if train_acc is not None:
            self.train_acc_data.append(train_acc)
            if len(self.train_acc_data) > 300: self.train_acc_data.pop(0)
            self.train_acc_curve.setData(self.train_acc_data)
        
        if test_acc is not None:
            self.test_acc_data.append(test_acc)
            if len(self.test_acc_data) > 300: self.test_acc_data.pop(0)
    ...
    def update_layer_analysis(self, layer_activities, layer_importances, layer_gate_states=None):
